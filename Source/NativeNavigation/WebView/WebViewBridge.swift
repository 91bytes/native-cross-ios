...
final class WebViewBridge {
    private let messageHandlerName = "nativeNavigation"
    ...
    private var userScript: WKUserScript {
        let url = Bundle.module.url(forResource: "native-navigation", withExtension: "js")!
    ...
    func visitLocation(_ location: URL, options: VisitOptions) {
        callJavaScript(function: "window.nativeNavigation.executeVisit", arguments: [
...
extension WebViewBridge: ScriptMessageHandlerDelegate {
    func scriptMessageHandlerDidReceiveMessage(_ scriptMessage: WKScriptMessage) {
        ...
        case .visitStarted:
            visitDelegate?.webView(self, didStartVisit: message.data["isPageRefresh"] as! Bool)
        case .visitFailed:
